...
const CLINIC_ADDRESS_EN =
  'Hawally - Block 4 - Al-Motasim St. - Specialized Clinics Center - 6th Floor';
const CLINIC_PHONE_EN = 'Tel.: 22650700 ext. 607';
const CLINIC_MOBILE_EN = 'Mobile & WhatsApp: 60008977';

const PAYMENT_METHOD_LABELS_EN: Record<
  InvoicePdfData['payments'][number]['method'],
  string
> = {
  LINK: 'LINK',
  KNET: 'KNET',
...
const ICON_PATHS: Record<string, string> = {
  ...
  person:
    '<circle cx="12" cy="8" r="4"/><path d="M4 21c1.5-5 5-7 8-7s6.5 2 8 7"/>',
  idCard:
    '<rect x="2" y="5" width="20" height="14" rx="2"/><circle cx="8" cy="12" r="2"/><path d="M13 10h6M13 14h4"/>',
  phone:
    '<path d="M6.6 10.8c1.4 2.8 3.8 5.2 6.6 6.6l2.2-2.2c.3-.3.7-.4 1-.2 1.1.4 2.3.6 3.5.6.6 0 1 .4 1 1V20c0 .6-.4 1-1 1C10.6 21 3 13.4 3 4c0-.6.4-1 1-1h3.2c.6 0 1 .4 1 1 0 1.2.2 2.4.6 3.5.1.4 0 .8-.2 1z"/>',
  mapPin:
...
function renderInvoiceCopy(
  ...
  const chargesRows = (invoice.additionalCharges || [])
    .map((charge) => {
      ...
      const priceDisplay =
        charge.chargeType === 'PERCENTAGE'
          ? formatMoney(charge.chargeValue) + '%'
          : formatMoney(charge.chargeValue);
  ...
  const replacementNote = invoice.replacedByInvoiceId
    ? `<div class="replacement-note">${isArabic
      ? 'تم استبدال هذه الفاتورة.'
  ...
  <div class="copy">
    ...
    <div class="title-row">
      <div class="invoice-title">
        <span class="arrow">&#8594;</span>${labels.invoice}<span class="arrow">&#8592;</span>
      </div>

      <div class="meta-inline">
        <span class="meta-chip">
          ${icon('document', 10)}
          <b>${labels.invoiceNo}:</b> ${escapeHtml(invoice.invoiceNumber)}
        </span>

        <span class="meta-chip">
          ${icon('calendar', 10)}
          <b>${labels.date}:</b> ${formatDate(invoice.issuedAt || invoice.createdAt)}
        </span>
      </div>
    </div>
    ...
    <div class="patient-row">
      ...
      <div class="p-field">
        ...
        <span class="p-value">
          ${invoice.patient.civilId ? escapeHtml(invoice.patient.civilId) : '&mdash;'}
        </span>
      </div>
      ...
      <div class="p-field">
        ...
        <span class="p-value">
          ${invoice.patient.phone ? escapeHtml(invoice.patient.phone) : '&mdash;'}
        </span>
      </div>
    </div>
    ...
    <div class="pay-row">
      ...
      <div class="pay-box">
        ...
        <span class="pay-value">
          ${lastPayment ? labels.paymentMethods[lastPayment.method] : '&mdash;'}
        </span>
      </div>
    </div>
