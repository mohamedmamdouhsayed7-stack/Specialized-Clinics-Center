...
export default function InvoiceDetail() {
  ...
  const statusMutation = useMutation({
    ...
    onError: (err: Error) => {
      ...
      showToast({
        type: 'error',
        message: err.message || t('feedback.invoiceStatusFailed'),
      });
    },
  });
  ...
  const paymentMutation = useMutation({
    ...
    onError: (err: Error) => {
      ...
      showToast({
        type: 'error',
        message: err.message || t('feedback.paymentRecordFailed'),
      });
    },
  });
  ...
  const reversePaymentMutation = useMutation({
    ...
    onError: (err: Error) => {
      ...
      showToast({
        type: 'error',
        message: err.message || t('feedback.paymentReverseFailed'),
      });
    },
  });
  ...
  const replacementMutation = useMutation({
    ...
    onError: (err: Error) => {
      ...
      showToast({
        type: 'error',
        message: err.message || t('feedback.invoiceReplacementFailed'),
      });
    },
  });
  ...
  // The invoice WhatsApp message is always Arabic.
   ...
   * WhatsApp directly on that number with the Arabic invoice message.
   */
  ...
  /*
   * Send Invoice PDF:
   * Gets the actual invoice PDF and opens the device/browser
   * share sheet so the user can select WhatsApp and then
   * select the patient's chat.
   */
  const shareInvoicePdfOnWhatsApp = () => {
    ...
    void (async () => {
      try {
        ...
        /*
         * The browser does not support file sharing.
         * Download the PDF so it can be attached manually.
         */
        const downloadUrl =
          window.URL.createObjectURL(file);
                ...
                >
                  ...
                  <p className="mt-4 text-xs leading-5 text-[#667085]">
                    {messageShareChannel ===
                      'whatsapp'
                      ? t(
                        'invoices.whatsappLimitation'
                      )
                      : t(
                        'invoices.smsLimitation'
                      )}
                  </p>
